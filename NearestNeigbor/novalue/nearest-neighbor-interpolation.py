...
# cv.imshow(''.join([str(img.shape[0]), 'x', str(img.shape[1])]), img)
...
expanded_img = cv.resize(img, expand_size, interpolation = cv.INTER_NEAREST)
...
reduced_img = cv.resize(img, reduce_size, interpolation = cv.INTER_NEAREST)
